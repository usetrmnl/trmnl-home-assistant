...
# Optional overrides from the add-on config, written by 01-init-environment.sh.
# Sourced last, but the passthrough list excludes the vars set above.
if [ -f /var/run/terminus.env ]; then
    source /var/run/terminus.env
fi
